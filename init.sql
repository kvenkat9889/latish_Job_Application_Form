CREATE TABLE applications (
    ref_no VARCHAR(120) PRIMARY KEY, -- Unique application reference number (e.g., APP-2025-12345)
    name VARCHAR(150) NOT NULL, -- Applicant's full name
    email VARCHAR(150) NOT NULL, -- Applicant's email address
    phone VARCHAR(75) NOT NULL, -- Applicant's phone number (up to 15 digits)
    gender VARCHAR(70) NOT NULL, -- Applicant's gender (Male, Female, Other)
    guardian_name VARCHAR(100) NOT NULL, -- Guardian's full name
    guardian_phone VARCHAR(25) NOT NULL, -- Guardian's phone number (up to 15 digits)
    job_type VARCHAR(200) NOT NULL, -- Job type (Fresher, Associate, Experienced)
    location VARCHAR(150) NOT NULL, -- Preferred work location (e.g., Hyderabad)
    status VARCHAR(120) DEFAULT 'New', -- Application status (New, Shortlisted, Approved, Rejected)
    ...
    ssc_certificate_name VARCHAR(300) NOT NULL, -- SSC certificate file name
    ssc_certificate_data TEXT NOT NULL, -- SSC certificate base64 data
    hsc_certificate_name VARCHAR(300) NOT NULL, -- HSC certificate file name
    hsc_certificate_data TEXT NOT NULL, -- HSC certificate base64 data
    graduation_certificate_name VARCHAR(300) NOT NULL, -- Graduation certificate file name
    graduation_certificate_data TEXT NOT NULL, -- Graduation certificate base64 data
    pg_certificate_name VARCHAR(300), -- PG certificate file name (optional)
    pg_certificate_data TEXT, -- PG certificate base64 data (optional)
    relieving_letter_name VARCHAR(300), -- Relieving letter file name (if has_work_exp)
    relieving_letter_data TEXT, -- Relieving letter base64 data (if has_work_exp)
    offer_letter_name VARCHAR(300), -- Offer letter file name (set by HR)
